...
func TestLoadFromStreamDetailBytes(t *testing.T) {
	withJSServer(t, func(t testing.TB, nc *nats.Conn, mgr *jsm.Manager, i *ntfclient.Instance) {
		...
		res, err := sysNc.Request("$SYS.REQ.SERVER.PING.JSZ", []byte(`{"streams":true,"config":true,"consumer":true}`), time.Second)
		checkErr(t, err, "ping failed")

		var jsz server.ServerAPIJszResponse
		checkErr(t, json.Unmarshal(res.Data, &jsz), "unmarshal failed")
		if len(jsz.Data.AccountDetails) == 0 {
			t.Fatalf("jsz should have at least one account")
		}
		if len(jsz.Data.AccountDetails[0].Streams) == 0 {
			t.Fatalf("jsz should have at least one stream")
		}

		sdb, err := json.Marshal(jsz.Data.AccountDetails[0].Streams[0])
...
func TestStream_LatestInformation(t *testing.T) {
	withJSServer(t, func(t testing.TB, nc *nats.Conn, mgr *jsm.Manager, srv *ntfclient.Instance) {
		stream, err := mgr.NewStream("q1", jsm.Subjects("in.q1"), jsm.FileStorage())
		checkErr(t, err, "create failed")
		...
		srv.StopServer(t, srv.Servers[0])
...
func TestStream_PurgeExt(t *testing.T) {
	withJSServer(t, func(t testing.TB, nc *nats.Conn, mgr *jsm.Manager, _ *ntfclient.Instance) {
		...
		checkCnt := func(t testing.TB, count uint64) {
			t.Helper()
			stats, err := stream.State()
			checkErr(t, err, "stats failed")
			if stats.Msgs != count {
...
func TestStream_ReadLastMessageForSubject(t *testing.T) {
	withJSServer(t, func(t testing.TB, nc *nats.Conn, mgr *jsm.Manager, _ *ntfclient.Instance) {
		...
		get := func(t testing.TB, subj string, expect string) {
			t.Helper()

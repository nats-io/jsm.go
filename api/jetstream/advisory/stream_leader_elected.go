...
func init() {
	err := event.RegisterTextCompactTemplate("io.nats.jetstream.advisory.v1.stream_leader_elected", `{{ .Time | ShortTime }} {{ .Stream }} elected leader {{ .Leader }} of {{ .Replicas | len }} peers`)
	if err != nil {
		panic(err)
	}

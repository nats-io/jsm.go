...
func DoReqAsync(ctx context.Context, req any, subj string, waitFor int, nc *nats.Conn, timeout time.Duration, log api.Logger, cb func([]byte)) error {
	...
	sub, err := nc.Subscribe(nc.NewRespInbox(), func(m *nats.Msg) {
		...
		if compressed {
			log.Debugf("<<< (%dB -> %dB) %s", len(m.Data), len(data), string(data))
		} else {
			log.Debugf("<<< (%dB) %s", len(data), string(data))
		}

		if m.Header != nil {
			log.Debugf("<<< Header: %+v", m.Header)
		}
...
func CurrentActiveServers(ctx context.Context, nc *nats.Conn, timeout time.Duration, log api.Logger) (int, error) {
	...
	err := DoReqAsync(ctx, nil, "$SYS.REQ.SERVER.PING", 1, nc, timeout, log, func(msg []byte) {

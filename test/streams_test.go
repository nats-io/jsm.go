...
func TestLoadFromStreamDetailBytes(t *testing.T) {
	withJSServer(t, func(t *testing.T, nc *nats.Conn, mgr *jsm.Manager) {
		...
		checkErr(t, err, "system connection failed")

		resp, err := sysNc.Request("$SYS.REQ.SERVER.PING.JSZ", []byte(`{"streams":true, "config":true, "consumer":true}`), time.Second)
		checkErr(t, err, "jsz failed")

		var jsz server.ServerAPIJszResponse
		err = json.Unmarshal(resp.Data, &jsz)
		checkErr(t, err, "json unmarshal failed")

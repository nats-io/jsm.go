...
func atomicityChecks() []Check {
	return []Check{
		...
		{
			ID: "behavior.sel_set_atomic", Section: "Behavior",
			Title: "sel.set is atomic vs concurrent sel.* callers",
			Modes: []string{"rw"},
			Run: func(ctx context.Context, h *Harness) (Status, string, error) {
				a := h.MintName("sel_at_a")
				b := h.MintName("sel_at_b")
				...
				if err != nil {
					return StatusFail, "seed selection: " + err.Error(), nil
				}

				var anomaly atomic.Int64
				var reads atomic.Int64
				var readErr atomic.Value
				...
				// Readers assert every Selected() observation is one of
				// the two legal names (never empty, never an old value).
				for i := 0; i < h.Concurrency; i++ {
					wg.Add(1)
					go func() {
						defer wg.Done()
						for {
							...
							got, selErr := h.Client.Selected(ctx)
							if selErr != nil {
								readErr.Store(selErr)
								return
							}
							reads.Add(1)
							if got != a && got != b {
				...
				go func() {
					...
					for i := 0; i < h.Iterations; i++ {
						...
						_, setErr := h.Client.SetSelected(ctx, target)
						if setErr != nil {
							return
						}
					}
				...
				if anomaly.Load() > 0 {
					return StatusFail, fmt.Sprintf("observed %d unexpected selection values in %d reads", anomaly.Load(), reads.Load()), nil
				}
				return StatusWarn, fmt.Sprintf("no selection anomalies observed in %d reads over %d writes (probabilistic)", reads.Load(), h.Iterations), nil
			},
		},
	}
}

...
func init() {
	err := event.RegisterTextCompactTemplate("io.nats.jetstream.advisory.v1.consumer_quorum_lost", `{{ .Time | ShortTime }} {{ .Stream }} > {{ .Consumer }} lost quorum of {{ .Replicas | len }} peers`)
	if err != nil {
		panic(err)
	}

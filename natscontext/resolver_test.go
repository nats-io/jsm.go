...
// gate: when Creds is explicitly set the user has opted out of
// nsc-driven resolution, so Save must not shell out to nsc even when
// URL is empty. The context is constructed directly (load=false) to
// exercise Save in isolation from the legacy-load path, which has
// always required nsc.
func TestNSCLookupOverrideOnSaveWithoutNsc(t *testing.T) {
	if runtime.GOOS == "windows" {
		t.Skip("PATH semantics differ on Windows; covered on unix")
	}
	...
	nctx, err := natscontext.New("legacy", false,
		natscontext.WithNscUrl("op/acct/user"),
		natscontext.WithCreds(overridePath),
	)
	if err != nil {
		t.Fatalf("new: %v", err)
	}
	...
	err = nctx.Save("")
	if err != nil {
		t.Fatalf("save with override and no nsc on PATH: %v", err)
	}

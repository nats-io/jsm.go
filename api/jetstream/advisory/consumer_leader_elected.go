...
func init() {
	err := event.RegisterTextCompactTemplate("io.nats.jetstream.advisory.v1.consumer_leader_elected", `{{ .Time | ShortTime }} {{ .Stream }} > {{ .Consumer }} elected leader {{ .Leader }} of {{ .Replicas | len }} peers`)
	if err != nil {
		panic(err)
	}
